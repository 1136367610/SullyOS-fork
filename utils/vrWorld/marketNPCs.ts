...
硬性长度：title 最多40字，words 最多600字（建议20～180字），event.story 最多600字。所有字符串内的英文双引号与换行必须按 JSON 转义；金额用数字，不带币种或单位。每个 actions 元素只表示一个动作，action 的值只选下面的一种，不要将字段说明中的 | 选项照抄为值。
动作格式（只写所需字段）：
